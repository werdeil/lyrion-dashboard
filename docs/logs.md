...
Two LRCLIB lines carry a URL, for the tracks worth a closer look. `lrclib: no record, catalogue search: https://lrclib.net/api/search?artist_name=Focus&track_name=Hocus+Pocus` replays the app's own query in a browser: it says whether the catalogue really holds nothing, or whether your tags spell the artist or the title differently from it. `lrclib: https://lrclib.net/tracks/22439347 is 419s, this track is 302s - its timings dropped` is the other frequent case — the right song, but a live or extended upload whose LRC would scroll against the wrong timeline, so its words are kept as plain lyrics and the karaoke is not. That browsable search does not filter on length the way the app does, so it will list records the app then refuses: the gap between the two is the diagnosis, not a bug.
