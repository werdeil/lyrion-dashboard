...
- **Web lyrics fallback** -- An auto-search switch queries LRCLIB, Musixmatch and Genius for every playing track: it fills in what the library is missing and upgrades plain text to a synced version when one exists. When both the library and the web have something for a track, the source line under the lyrics switches between them, so a wrong text in your tags can be read against the web's.
