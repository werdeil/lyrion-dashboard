...
- **Web lyrics fallback** -- An auto-search switch queries the web (LRCLIB, Musixmatch, Genius) for every playing track: it fills in lyrics the library is missing, and upgrades the library's plain text to a synced (karaoke) version when one is found. While it's on, a retry button re-runs the search for the current track, bypassing the cache.
