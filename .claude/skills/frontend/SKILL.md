...
The caption (`.cover-zoom-meta`) is a plaque hugging its text near the picture's lower edge, not a band across it: absolutely positioned with `width: fit-content` and auto margins. Its tint and `backdrop-filter` blur sit on a `::before` layer, rounded and inset inside the caption's padding, whose own `filter: blur()` feathers that rounded edge outwards — so the plaque has no outline, and the frosted artwork fades with it. The layer carries both because a `filter` on the caption would blur the text too, and the text is positioned to paint over the layer rather than under it.
