...
The chip floats over the lyrics box's lower-right corner, and the box owes it `--source-clearance` of bottom padding — including in `lrc-mode`, which otherwise zeroes it. Without that, `scrollTop` clamps with the last line flush to the bottom edge and the chip covers the line being sung at the end of a song; the clearance is a shade wider than the chip's height plus its inset, so a full line of lyrics never rests under it.
