...
- **Now Playing** -- Détecte automatiquement le lecteur en cours de lecture et affiche sa piste (pochette, titre, artiste, album), rafraîchi via l'API JSON-RPC de Lyrion. La couleur d'accent s'adapte automatiquement à la pochette. Un clic sur la pochette l'agrandit sur toute la carte de lecture — les statistiques restent lisibles à côté — à la résolution de l'image d'origine ; un clic n'importe où ou Échap la referme.
